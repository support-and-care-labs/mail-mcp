...
== Option 1: HTTP/SSE with Docker (Production-like)

Test the MCP server via HTTP/SSE transport, the same way MCP clients connect in production:
...
NOTE: For full MCP protocol testing (tool calls via SSE), use an MCP client like Claude Code:
...
The official MCP Inspector provides a web UI for testing MCP servers via stdio transport.
This is useful for development when running the server directly without Docker.
...
* xref:usage:mcp-server.adoc[MCP Server Setup and Usage]
